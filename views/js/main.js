...
//randomPizza and pizzaSize saved as variable with getElementById for quiker work
var randomPizza = document.getElementById("randomPizzas");
var pizzasSize = document.getElementById("pizzaSize");
...
var resizePizzas = function(size) { 
  ...
  function changeSliderLabel(size) {
    switch(size) {
      case "1":
        pizzasSize.innerHTML = "Small";
        return;
      case "2":
        pizzasSize.innerHTML = "Medium";
        return;
      case "3":
        pizzasSize.innerHTML = "Large";
        return;
      default:
  ...
  function determineDx (elem, size) {
    ...
    var windowwidth = randomPizza.offsetWidth;
  ...
  //save "randomPizzaContainer" as variable for quicker work
  var rpizzaContainer = document.getElementsByClassName("randomPizzaContainer");
  // Iterates through pizza elements on the page and changes their widths
  function changePizzaSizes(size) {
    // saving rpizzaContainer length
    var pizzaContainerL = rpizzaContainer.length;
    var dx = determineDx(rpizzaContainer[0], size);
    var newwidth = (rpizzaContainer[0].offsetWidth + dx) + 'px';
    for (var i = 0; i < pizzaContainerL; i++) {
      rpizzaContainer[i].style.width = newwidth;
    }
  }
...
//using requestAniomationFarme to optimize anaimtion
...
function updatePositions() {
  ...
  //saving scroll position in a variable for quiker animation
  var scrollPosition = document.body.scrollTop / 1250;
  //saving math operations to prevent numerous counting in for loop
  var cosScroll = Math.cos(scrollPosition);
  var sinScroll = Math.sin(scrollPosition);
  for (var i = 0; i < items.length; i++) {
    var phase = (sinScroll*Math.cos(i % 5)) + (cosScroll*Math.sin(i % 5));
    var item = items[i];
    //we need to change only x coordinates
    var translateX = item.translateX + 100 * phase;
    var translateY = item.translateY; 
    //using translate3d instead of style.left makes the animation faster and smoother
    ...
    item.style.msTransform = 'translate3d(' + translateX + 'px, ' + translateY + 'px, 0px)';
    item.style.OTransform = 'translate3d(' + translateX + 'px, ' + translateY + 'px, 0px)';
...
document.addEventListener('DOMContentLoaded', function() {
  var cols = 8;
  var s = 256;
  // saving "movingPizzas1" ina  variable
  ...
  for (var i = 0; i < 200; i++) {
    ...
    // using translate3d for quicker and more resource effective animation
    ...
    elem.style.msTransform = 'translate3d(0px, ' + elem.translateY + 'px, 0px)';
    elem.style.OTransform = 'translate3d(0px, ' + elem.translateY + 'px, 0px)';
